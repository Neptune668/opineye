...
为支撑单人开发下的独立交付与测试，模块间遵循「面向接口编程 + 依赖注入（DI）」原则，禁止跨模块直接 `import` 具体实现：

- **依赖倒置**：上层模块仅依赖下层模块暴露的抽象接口（`Protocol` 或 `ABC`），不感知具体实现。实现可在不修改调用方的前提下替换（如 `LLMClient` 的规则实现/大模型实现）。
- **依赖注入**：所有跨模块依赖通过构造函数或 FastAPI `Depends` 注入，由装配层（`app/dependencies.py`）统一组装。禁止在模块内部 `new`/实例化其他模块对象。
- **仓储模式（Repository）**：数据库访问封装为仓储接口（如 `SearchTaskRepository`），Service 依赖接口而非 ORM 会话，便于用内存实现做单元测试。
- **端口-适配器**：外部依赖（采集源、LLM、消息推送）以「端口接口 + 适配器实现」组织，默认提供「本地/离线适配器」，生产可替换为真实适配器。
- **事件解耦**：跨模块协作优先通过「领域事件 + 消息队列/事件总线」而非直接调用，降低耦合（如报告生成完成 → 发布 `graph_ready` 事件，图谱模块订阅）。

**接口抽象清单（冻结，作为任务间契约）**：

| 接口 | 提供方 | 依赖方 | 说明 |
|------|--------|--------|------|
| `ConfigService` | T3 | 全局 | 配置读写与热更新 |
| `ProcessManager` | T4 | T5/T6/T8/T10 | 应用启停、状态、输出 |
| `AppOutputReader` | T5 | T10 | 输出与日志读取 |
| `SearchEngine` | T6 | T7 | 检索与分析入口 |
| `ReportWriter` | T7 | T9/T12 | 报告生成 |
| `ForumCollector` | T8 | T12 | 论坛采集控制 |
| `GraphStore` | T9 | T12 | 图谱读写查询 |
| `LLMClient` | T11 | T6 | 可插拔大模型适配 |
| `EventBus` | T1 | 全局 | 领域事件发布订阅 |

> 接口一经冻结，各任务即可按「先定义接口 → 用 Mock 替身开发 → 最后替换为真实实现」的顺序独立推进，实现真正的解耦合。

### 4.8 核心接口签名定义

> 以下签名为任务间冻结契约，采用 Python `Protocol` 描述；实现类无需继承，仅需满足方法签名。参数与返回值类型即契约，改动须升版本。
...
class DomainEvent:
    type: str            # app_status / app_output / forum_log / system_status / graph_ready / error
...
class ConfigService(Protocol):
    ...
    def save(self, cfg: Config, expected_version: int) -> Config: ...  # 版本不匹配抛 VersionConflict
    def watch(self, key: str, callback: Callable[[Any], None]) -> None: ...


# ---- T4 ProcessManager（进程/应用管理）----
class ProcessManager(Protocol):
    def start(self, app_name: str) -> AppStatus: ...      # 状态非法抛 InvalidStateError
    def stop(self, app_name: str) -> AppStatus: ...       # 幂等
    def status(self, app_name: str | None = None) -> dict[str, AppStatus]: ...


# ---- T5 AppOutputReader（输出/日志读取）----
class AppOutputReader(Protocol):
    def read_output(self, app_name: str) -> str: ...      # outputs/{app}/latest.txt
    def read_log(self, app_name: str, tail: int = 200) -> list[str]: ...


# ---- T6 Collector（数据采集）----
class Collector(Protocol):
    def collect(self, request: SearchRequest) -> list[SourceItem]: ...  # 三路并行采集

@dataclass(frozen=True)
class SourceItem:
    source_type: str                   # news/image/video/forum_post/internal_data
    title: str
    url: str | None = None
...
class SearchRequest:
    query: str
    source_types: list[str]           # news/image/video/forum_post/internal_data

@dataclass(frozen=True)
class SearchResult:
    task_id: str
    sources: list[SourceItem]
    analysis: AnalysisOutput          # 关键词/情绪/时间线/观点


# ---- T8 ReportWriter（报告生成）----
class ReportWriter(Protocol):
    def write(self, result: SearchResult, report_id: str) -> str: ...  # 返回 report.md 路径


# ---- T9 ForumCollector（论坛采集）----
...
# ---- T12 LLMClient（可插拔大模型）----
class LLMClient(Protocol):
    def available(self) -> bool: ...                      # 无密钥返回 False → 回退规则
...
class GraphRepository(Protocol):
    ...
    def query(self, report_id: str, cond: GraphQuery) -> GraphData: ...

class ForumLogRepository(Protocol):
    def append(self, entry: ForumLogEntry) -> None: ...
    def query_by_date(self, date: str) -> list[ForumLogEntry]: ...
```
...
class ForumState(str, Enum):
    IDLE="idle"; RUNNING="running"; STOPPED="stopped"; FAILED="failed"

class SystemState(str, Enum):
    OFFLINE="offline"; STARTING="starting"; ONLINE="online"; SHUTTING_DOWN="shutting_down"

class SourceType(str, Enum):
    NEWS="news"; IMAGE="image"; VIDEO="video"; FORUM_POST="forum_post"; INTERNAL_DATA="internal_data"
```
...
| 异常 | 触发场景 | 映射错误码 |
|------|----------|-----------|
| `ValidationError` | 参数非法 | 1001 |
| `NotFoundError` | 资源不存在 | 1002 |
| `InvalidStateError` | 状态不允许该操作 | 2001 |
| `VersionConflictError` | 配置版本不匹配 | 2001 |
| `TaskFailedError` | 任务执行失败 | 4001 |
| `StorageError` | 文件/DB 读写失败 | 3001/3002 |
...
> 说明：以下任务拆分遵循「依赖前置、模块内聚、接口解耦、可独立验收」原则。每个任务为一个可交付的增量（Increment），通过 4.7 节冻结的接口契约与其他任务解耦；任务间依赖关系见 11.2。

### 11.1 任务清单

| 编号 | 任务名称 | 所属模块 | 交付物 | 依赖（仅接口） | 预估工时（人日） |
|------|----------|----------|--------|----------------|------------------|
| T1 | 项目骨架与基础设施 | 全局 | 目录结构、`requirements.txt`、`.env.example`、`config.json`、`main.py` 可启动、日志封装、`EventBus` 接口、依赖装配层 | - | 1 |
| T2 | 数据库建模与迁移 | 全局 | 6 张表 DDL、SQLAlchemy 模型、Alembic 迁移脚本、`models/`、各仓储接口定义 | T1 | 1 |
| T3 | 配置模块 | 配置 | `ConfigService` 接口与实现、`GET/POST /api/config`、热更新、版本乐观锁 | T1 | 1 |
| T4 | 进程管理模块 | 进程管理 | `ProcessManager` 接口与实现、Celery 应用初始化、应用状态机、`/api/status`、`/api/start/{app_name}`、`/api/stop/{app_name}` | T1 | 3 |
| T5 | 单功能应用输出查看 | 进程管理 | `AppOutputReader` 接口与实现、`/api/output/{app_name}`、`/api/test_log/{app_name}` | T4（接口） | 1 |
| T6 | 数据采集模块 | 检索分析 | `Collector` 接口与实现、三类采集源适配器（news/image/video/forum_post/internal_data）、Celery 采集任务体 | T2（仓储接口）、T4（接口） | 3 |
| T7 | 检索分析引擎（规则模式） | 检索分析 | `SearchEngine` 接口与实现、`analysis/`、`/api/search` | T2（仓储接口）、T6（接口） | 4 |
| T8 | 报告生成 | 检索分析 | `ReportWriter` 接口与实现、`reports/{id}/report.md`、报告元数据落库 | T7（接口） | 2 |
| T9 | 论坛模块 | 论坛 | `ForumCollector` 接口与实现、`/api/forum/*` 四接口、日志归档 | T4（接口） | 2 |
| T10 | 图谱模块 | 图谱 | `GraphStore` 接口与实现、`graph_builder`、`/api/graph/*` 三接口 | T2（仓储接口）、T8（接口） | 3 |
| T11 | 系统启停与实时消息 | 控制层 | `/api/system/*`、WebSocket 六类消息推送、事件订阅器 | T4（接口）、T5（接口） | 2 |
| T12 | 可选 LLM 增强 | 检索分析 | `LLMClient` 接口 + 规则/LLM 双实现、自动回退 | T7（接口） | 2 |
| T13 | 示例数据与主题案例 | 全局 | 内部数据样本、完整主题检索→分析→图谱→报告演示 | T8、T9、T10（接口） | 2 |
| T14 | 接口文档与联调 | 全局 | OpenAPI 文档、接口说明、验收走查 | T3~T11（接口） | 1 |

### 11.2 依赖关系

> 依赖仅指向「接口契约」，不指向具体实现。箭头表示「使用对方接口」。

```mermaid
flowchart LR
    T1["T1 骨架<br/>(EventBus)"] --> T2["T2 数据库<br/>(仓储接口)"]
    T1 --> T3["T3 配置<br/>(ConfigService)"]
    T1 --> T4["T4 进程管理<br/>(ProcessManager)"]
    T2 --> T6["T6 数据采集<br/>(Collector)"]
    T4 --> T5["T5 输出查看<br/>(AppOutputReader)"]
    T4 --> T6
    T4 --> T9["T9 论坛<br/>(ForumCollector)"]
    T4 --> T11["T11 系统启停/WS"]
    T5 --> T11
    T6 --> T7["T7 检索分析<br/>(SearchEngine)"]
    T7 --> T8["T8 报告<br/>(ReportWriter)"]
    T7 --> T12["T12 LLM 增强<br/>(LLMClient)"]
    T2 --> T10["T10 图谱<br/>(GraphStore)"]
    T8 --> T10
    T8 --> T13["T13 示例/案例"]
    T9 --> T13
    T10 --> T13
...
| M3 核心分析 | T6, T7, T8, T12 | 可发起主题检索并生成结构化报告 |
| M4 论坛与图谱 | T9, T10 | 可论坛采集、查看最新/历史日志；可查看图谱 |
| M5 系统集成 | T11, T13, T14 | 系统启停、实时消息、完整主题案例演示通过 |
...
| T9 | 能启动论坛采集，查看最新日志与历史日志 |
| T7/T8 | 能发起主题检索，看到结构化分析结果（8 节报告） |
| T10 | 能查看最新图谱与指定报告图谱，按节点/关系查询 |
| T3 | 能更新配置并即时生效 |
| T6 | 能从至少一类来源采集到内容并落库（支持内部数据源离线演示） |
| T13 | 能完成至少一个完整主题的检索、分析、图谱查看、结果展示演示 |

### 11.5 任务级解耦细节

> 每个任务按「定义接口 → 实现 → 用 Mock 替身自测 → 装配」四步推进，保证不依赖其他任务的真实实现即可交付与测试。

| 任务 | 对外接口（输出契约） | 依赖的 Mock 替身（输入替身） | 独立测试方式 |
|------|----------------------|------------------------------|--------------|
| T1 | `EventBus`、日志接口 | 无 | 启动服务、验证事件发布/订阅 |
| T2 | 仓储接口（SearchTask/Source/Report/GraphNode/GraphEdge/ForumLog） | 无 | Alembic 迁移 + 仓储 CRUD 单测 |
| T3 | `ConfigService`（read/save/watch） | 内存文件 | 单测读写 + 热更新通知 |
| T4 | `ProcessManager`（start/stop/status） | 进程内存状态 + 空任务占位 | 状态机流转单测 + 幂等测试 |
| T5 | `AppOutputReader`（read_output/read_log） | `ProcessManager` Mock | 用预置日志文件验证读取 |
| T6 | `Collector`（collect） | `仓储接口` Mock | 用预置内部数据源验证采集与落库 |
| T7 | `SearchEngine`（search） | `Collector` Mock + `LLMClient` 规则版 | 用固定语料断言分析结果 |
| T8 | `ReportWriter`（write） | `SearchEngine` 返回的固定分析结果 | 断言报告 8 节结构完整 |
| T9 | `ForumCollector`（start/stop/log/history） | `ProcessManager` Mock | 日志归档与历史查询单测 |
| T10 | `GraphStore`（load/query/latest） | `ReportWriter` 产出的固定 graph.json | 断言节点/边/关系查询 |
| T11 | 系统状态接口 + 事件订阅器 | `ProcessManager`/`AppOutputReader` Mock | WebSocket 消息类型覆盖测试 |
| T12 | `LLMClient`（双实现） | `SearchEngine` 规则版 | 无密钥回退 + 有密钥增强两路测试 |
| T13 | 示例数据 + 演示脚本 | 真实实现（此时已全部就绪） | 端到端跑通完整主题 |
| T14 | 接口说明文档 | 真实实现 | 全接口回归 + 验收走查 |

**解耦装配规则**：

- 所有接口注册在 `app/dependencies.py`，通过 `Depends` 注入；任务实现替换仅改装配，不改调用方。
- Mock 替身与真实实现实现同一接口，测试环境通过配置开关切换（如 `REPO_BACKEND=memory|mysql`）。
- 跨模块协作一律通过 `EventBus` 发布事件，订阅方解耦（如 T7 完成报告后发布 `report_ready`，T9 订阅后生成图谱）。
...
| 阶段三 | T6 → T7 → T8 | 采集→分析→报告生成结构化报告（M3） |
| 阶段四 | T9 → T10 | 论坛采集、图谱查看通过（M4） |
| 阶段五 | T11 → T12 → T13 → T14 | 系统启停、WS、完整案例演示、接口联调（M5） |

**单人开发约束**：

- 一次只推进一个任务，任务未通过自身验收标准（见 11.4）不进入下一个任务。
- 每个任务优先定义并冻结其对外接口（见 11.5），再用 Mock 替身完成自测，最后在装配层接入真实实现。
- T12（LLM 增强）为可选任务，若工期紧张可顺延至 M5 之后单独补做，不阻塞主线交付。
