...
| T6 | 检索分析引擎（规则模式） | 检索分析 | `SearchEngine` 接口与实现、`analysis/`、`/api/search` | T2（仓储接口）、T4（接口） | 4 |
| T7 | 报告生成 | 检索分析 | `ReportWriter` 接口与实现、`reports/{id}/report.md`、报告元数据落库 | T6（接口） | 2 |
| T8 | 论坛模块 | 论坛 | `ForumCollector` 接口与实现、`/api/forum/*` 四接口、日志归档 | T4（接口） | 2 |
| T9 | 图谱模块 | 图谱 | `GraphStore` 接口与实现、`graph_builder`、`/api/graph/*` 三接口 | T2（仓储接口）、T7（接口） | 3 |
| T10 | 系统启停与实时消息 | 控制层 | `/api/system/*`、WebSocket 六类消息推送、事件订阅器 | T4（接口）、T5（接口） | 2 |
| T11 | 可选 LLM 增强 | 检索分析 | `LLMClient` 接口 + 规则/LLM 双实现、自动回退 | T6（接口） | 2 |
| T12 | 示例数据与主题案例 | 全局 | 内部数据样本、完整主题检索→分析→图谱→报告演示 | T7、T8、T9（接口） | 2 |
| T13 | 接口文档与联调 | 全局 | OpenAPI 文档、接口说明、验收走查 | T3~T10（接口） | 1 |
...
flowchart LR
    ...
    T2 --> T6["T6 检索分析<br/>(SearchEngine)"]
    T4 --> T5["T5 输出查看<br/>(AppOutputReader)"]
    T4 --> T6
    T4 --> T8["T8 论坛<br/>(ForumCollector)"]
    T4 --> T10["T10 系统启停/WS"]
    T5 --> T10
    T6 --> T7["T7 报告<br/>(ReportWriter)"]
    T6 --> T11["T11 LLM 增强<br/>(LLMClient)"]
    T2 --> T9["T9 图谱<br/>(GraphStore)"]
    T7 --> T9
    T7 --> T12["T12 示例/案例"]
    T8 --> T12
    T9 --> T12
...
| T4 | `ProcessManager`（start/stop/status） | `Celery` 本地 worker（或 `Celery` 的 eager 模式） | 状态机流转单测 + 幂等测试 |
| T5 | `AppOutputReader`（read_output/read_log） | `ProcessManager` Mock | 用预置日志文件验证读取 |
| T6 | `SearchEngine`（search） | `仓储接口` Mock + `LLMClient` 规则版 | 用固定语料断言分析结果 |
| T7 | `ReportWriter`（write） | `SearchEngine` 返回的固定分析结果 | 断言报告 8 节结构完整 |
| T8 | `ForumCollector`（start/stop/log/history） | `ProcessManager` Mock | 日志归档与历史查询单测 |
| T9 | `GraphStore`（load/query/latest） | `ReportWriter` 产出的固定 graph.json | 断言节点/边/关系查询 |
| T10 | 系统状态接口 + 事件订阅器 | `ProcessManager`/`AppOutputReader` Mock | WebSocket 消息类型覆盖测试 |
| T11 | `LLMClient`（双实现） | `SearchEngine` 规则版 | 无密钥回退 + 有密钥增强两路测试 |
| T12 | 示例数据 + 演示脚本 | 真实实现（此时已全部就绪） | 端到端跑通完整主题 |
| T13 | 接口说明文档 | 真实实现 | 全接口回归 + 验收走查 |
